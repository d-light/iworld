	...
	$.getJSON(SITEURL+'/index.php?act=cart&op=ajax_load&callback=?', function(result){
	    ...
	    if(result){
	       	...
	       	if(result.cart_goods_num >0){
			    ...
			    $("#rtoobar_cart_count").html(result.cart_goods_num).show();//更新右边购物车修复
	      } else {
	      	html="<div class='no-order'><span>您的购物车中暂无商品，赶快选择心爱的商品吧！</span></div>";
	      	obj.find('.incart-goods').html(html);
	      	obj.find('.total-price').html('');
			
			$(".addcart-goods-num").remove();//更新右边购物车修复
			$("#rtoobar_cart_count").html("").hide();//更新右边购物车修复
	      }
	   }
	});
}			 
    ...
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            ...
            if(result.quantity == 0){
				...
				$(".cart-list").html('<li><dl><dd style="text-align: center; ">暂无商品</dd></dl></li>');//更新右边购物车修复
                $('div[ncType="rtoolbar_total_price"]').html("");//更新右边购物车修复
                $("#rtoobar_cart_count").html("").hide();//更新右边购物车修复
            }else{
				...
				$('div[ncType="rtoolbar_total_price"]').html("共计金额：<em class='goods-price'>&yen;"+result.amount+"</em>");//更新右边购物车修复
                $("#rtoobar_cart_count").html(a.quantity);//更新右边购物车修复
                if($("#rtoolbar_cartlist > ul").children().size()!=result.quantity){//更新右边购物车修复
	            $("#rtoolbar_cartlist").load("index.php?act=cart&op=ajax_load&type=html");//更新右边购物车修复
                return;}//更新右边购物车修复
            }
        }else{
            alert(result.msg);
